...
log_failure_details() {
    ...
    # Target Logs
    echo "${CYAN}  Logs from target (statefulsets/target-statefulset):${NC}"
    kubectl logs -n "$TARGET_NAMESPACE" statefulsets/target-statefulset  --tail=30 | sed 's/^/    /' || echo "${YELLOW}    - Could not retrieve target pod logs${NC}"
