...
# 1. Apply target statefulset
apply_template "$MANIFEST_DIR/test-statefulset/target-statefulset.yaml" "$NAMESPACE"
kubectl wait --for=condition=Ready pods -l app=target-statefulset -n $NAMESPACE --timeout=${TIMEOUT}s

# 2. Apply keda ScaledObject in KEDA for Target  
apply_template "$MANIFEST_DIR/test-statefulset/keda-scaledObject-Target.yaml" "$NAMESPACE"
kubectl wait --for=condition=Ready scaledobject/target-scaled-object -n $NAMESPACE --timeout=${TIMEOUT}s

# 3. Apply ElastiService
apply_template "$MANIFEST_DIR/test-statefulset/target-elastiservice.yaml" "$NAMESPACE"
